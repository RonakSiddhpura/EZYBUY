...
function Filter({priceOrder,setPriceOrder}) {
...
function setSearchProduct(){
    let productItems = (product.filter((obj)=> obj.title.toUpperCase().includes(searchkey.toUpperCase())))
    setPriceOrder([...productItems])
//    console.log((product.filter((obj)=> obj.title.includes(searchkey))))
   }


   function LowToHigh(){
    let dataSorted = [];
   let sortedArr = priceOrder.sort((a, b) => {
     if (parseInt(a.price) < parseInt(b.price)) return -1;
     if (parseInt(a.price) > parseInt(b.price)) return 1;
     return 0;
 }); 
     setPriceOrder([...sortedArr])
 }
 function HighToLow(){
    let dataSorted = [];
   let sortedArr = priceOrder.sort((a, b) => {
...
function setSearchProductCategory(){
    
    let productItems = (product.filter((obj) => obj.category.toLowerCase().includes(filterdata.toLowerCase())))
    setPriceOrder([...productItems])
//    console.log((product.filter((obj)=> obj.title.includes(searchkey))))
   }
    return (
            ...
            <div className=' container mx-auto px-4 mt-5 '>
                <div className="p-5 rounded-lg bg-gray-100 drop-shadow-xl border border-gray-200"
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <button className="px-4 py-2 bg-gray-50hover:bg-gray-200 text-gray-800 text-sm font-medium rounded-md" onClick={()=>{setPriceOrder(product)}} style={{ color: mode === 'dark' ? 'white' : '' }}>
                            Reset Filter
                        </button>
                    </div>
                    <div>
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
                            <select defaultValue={filterdata} onChange={(e)=> {
